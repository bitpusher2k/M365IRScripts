...
Write-Output "`nRunning 18-Search-InboxRuleChanges.ps1... First pass..."
...
Write-Output "`nRunning 20-Get-ForwardingSettings.ps1..."
& "$PSScriptRoot\20-Get-ForwardingSettings.ps1" -OutputPath $OutputPath

Write-Output "`nRunning 21-Get-MailboxPermissions.ps1..."
...
$Response = Read-Host "Retrieve often relevant UAL entries for past $DaysAgo days? (Y/N - default N)"
if ($Response -eq 'Y') {
    Write-Output "`nRunning 15-Search-UnifiedAuditLogIR.ps1..."
    & "$PSScriptRoot\15-Search-UnifiedAuditLogIR.ps1" -OutputPath $OutputPath -DaysAgo $DaysAgo
}

$Response = Read-Host "Retrieve all available UAL entries for past $DaysAgo days? (Y/N - default N)"
if ($Response -eq 'Y') {
...
Write-Output "`nRunning 18-Search-InboxRuleChanges.ps1... Second pass (often gets info when first is blank)..."
& "$PSScriptRoot\18-Search-InboxRuleChanges.ps1" -OutputPath $OutputPath -DaysAgo $DaysAgo

$Response = Read-Host "Run CrowdStrike Reporting Tool for Azure (CRT - will prompt for Exchange Online and Azure AD auth and disconnect after)? (Y/N - default N)"
if ($Response -eq 'Y') {
    Write-Output "`nDownloading and Running Get-CRTReport.ps1..."
    ...
    & "$PSScriptRoot\Get-CRTReport.ps1" -WorkingDirectory $OutputPath -Interactive
}
