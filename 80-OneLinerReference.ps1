...
Write-Output `n '-----------------------------------------------'
Write-Output 'Revoke all sessions for all M365 users'
Write-Output '-----------------------------------------------'
Write-Output `n '   $userlist = get-mguser -all | where {$_.UserPrincipalName -ne $(Get-MgContext).account} ; foreach ($user in $userlist) {revoke-mgusersigninsession -userid $user.id}'
Write-Output '   '
Write-Output `n '-----------------------------------------------'
...
Write-Output '   Get-Mailbox -identity "$mailbox" | Select UserPrincipalName, Alias, DisplayName, Name, IsMailboxEnabled, SKUAssigned, IsDirSynced, WhenCreated, WhenChanged | fl ; Get-InboxRule -IncludeHidden –Mailbox "$mailbox" | ft ; Get-InboxRule -IncludeHidden –Mailbox "$mailbox" | Select MailboxOwnerId, Name, Enabled, RuleIdentity, Description, DeleteMessage, MoveToFolder, MarkAsRead | fl'
Write-Output '   Get-Mailbox -identity "$mailbox" | Select UserPrincipalName, Alias, DisplayName, Name, IsMailboxEnabled, SKUAssigned, IsDirSynced, WhenCreated, WhenChanged | fl ; Get-InboxRule -IncludeHidden –Mailbox "$mailbox" | Where {$_.Name -ne "Junk E-mail Rule"} | ft ; Get-InboxRule -IncludeHidden –Mailbox "$mailbox" | Where {$_.Name -ne "Junk E-mail Rule"} | Select MailboxOwnerId, Name, Enabled, RuleIdentity, Description, DeleteMessage, MoveToFolder, MarkAsRead | fl'
Write-Output '   Remove-InboxRule -Mailbox "$mailbox" -Identity "XXX" -Confirm:$false ; Get-InboxRule -IncludeHidden –Mailbox "$mailbox"'
Write-Output '   '
Write-Output `n '-----------------------------------------------'
