...
#Requires -Modules Microsoft.Graph.Beta.Reports, Microsoft.Graph.Identity.DirectoryManagement
...
Assert-M365Connection -RequireGraph -GraphScopes @("AuditLog.Read.All", "Domain.Read.All", "Organization.Read.All")
...
if (!$LicenseBool) {
    Write-Output "`nRetrieval of Entra ID sign-in logs through Microsoft Graph API requires Entra ID premium license (P1 or P2)"
    ...
    $EntraSignInLogsNonInteractive = Get-MgBetaAuditLogSignIn -All -Filter "(createdDateTime gt $(($StartDate).ToString('yyyy-MM-dd'))) and (signInEventTypes/any(t: t ne 'interactiveUser'))"
