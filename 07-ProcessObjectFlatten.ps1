...
param(
    ...
    [string]$outputFile = "File_Processed.csv",
    [string]$function = "EvotecIT,simple", # Can be "iRon", "EvotecIT", "RamblingCookieMonster", "proxb", "simple", "all" (to process using every function), or a comma-separated list
    [string]$scriptName = "ProcessObjectFlatten",
...
Write-Output "Imported data $(($data | Measure-Object -Character).characters) characters long."
$JsonData = $Data | ConvertFrom-Json
Write-Output "Json data $($JsonData.count) records long."

Write-Output "`nStarting recursive flattening of $inputFile. Recursive JSON flattening not recommended for files larger than around 10mb (10,000,000 characters)..."
...
if ($function -match "iRon" -or $function -eq "all") {
    ...
    $FlatData = $JsonData | Flatten-Object -Base 1 -Depth 20 -Uncut 20
    $FlatData | Export-Csv -Path "$outputPath" -NoTypeInformation
    Write-Output "`n$outputPath written."
...
if ($function -match "EvotecIT" -or $function -eq "all") {
    ...
    $FlatData = $JsonData | ConvertTo-FlatObject -Base 1 -Depth 20
    $FlatData | Export-Csv -Path "$outputPath" -NoTypeInformation
...
if ($function -match "RamblingCookieMonster" -or $function -eq "all") {
    ...
    $FlatData = $JsonData | ConvertTo-FlatObject2 -MaxDepth 20
    $FlatData | Export-Csv -Path "$outputPath" -NoTypeInformation
    Write-Output "`n$outputPath written."
...
if ($function -match "proxb" -or $function -eq "all") {
    ...
    $FlatData = $JsonData | Convert-OutputForCSV -OutputPropertyType "Comma"
    $FlatData | Export-Csv -Path "$outputPath" -NoTypeInformation
    Write-Output "`n$outputPath written."
